...
class UsersRouter:
        ...
        def website_users_index():
            try:
                ...
                if user['password'] != body['password']:
                    self.config.db.users.get_user_by_id(
                        id=user['_id'], login=True)
    ...
    def assign_profile_router(self):
        ...
        def website_profile_index():
            ...
            user_data = self.config.db.users.get_user_by_id(current_user_id)
            user_posts = self.config.db.users.get_user_posts(current_user_id)
            return render_template(
                'website/profile/index.html',
                content=self.config.website_content,
                lang=lang or "en",
