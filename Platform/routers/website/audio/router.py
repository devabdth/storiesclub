...
class AudioRouter:
    def __init__(self, app: Flask, config):
        ...
        self.assign_audio_update()

    def assign_audio_update(self):
        @self.app.route('/audio/', methods=["PATCH", "DELETE"])
        def audio_update():
            if request.method == "PATCH":
                ...
                try:
                    id_ = body['id']
                    del body['id']
                    res = self.config.db.audios.update_audio(
                        id=id_, payload=body)
                    if res:
            ...
            if request.method == ["DELETE"]:
                ...
                try:
                    res = self.config.db.audios.delete_audio_by_id(id_)
                    if res:
                        return self.app.response_class(status=200)
                except Exception as e:
                    print(e)
